...
const Calendar: React.FC<CalendarProps> = ({  onDateChange, onNext }) => {
  ...
  const [selectedDay, setSelectedDay] = useState<number>(18); // Default to 18 as in the original code
  ...
  // Mock available days - in a real app, this would come from your backend
  const availableDays = [14, 15, 16, 18, 19, 20, 21, 25, 26, 27, 28, 29, 30];
    ...
    <div className="main-content">
      ...
      <div className="days">
        ...
        {days.map(day => (
          <div 
            key={day}
            className={`day ${day === selectedDay ? 'selected' : ''} ${availableDays.includes(day) ? 'available' : ''}`}
            onClick={() => availableDays.includes(day) ? handleDayClick(day) : null}
          >
            {day}
          </div>
        ))}
